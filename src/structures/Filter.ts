...
export enum Lang { ENGLISH, RUSSIAN };
export enum Method { CLASSIC, STRICT };
...
export class Filter {
    ...
    constructor() {
        this.methods
            .set(Method.CLASSIC, (word: string, langs: Lang[]): boolean => {
                if (langs.includes(Lang.RUSSIAN) && !ru.exclude.includes(word) && ru.include.filter((w) => word === w || word.includes(w)).length != 0) {
                    return true;
                }
                if (langs.includes(Lang.ENGLISH) && !en.exclude.includes(word) && en.include.filter((w) => word === w || word.includes(w)).length != 0) {
                    return true;
                }
                return false;
            })
            .set(Method.STRICT, (word: string, langs: Lang[]): boolean => {
                ...
                if (langs.includes(Lang.RUSSIAN) && !ru.exclude.includes(word)) {
                    if (ru.include.filter((w) => word === w || word.includes(w)).length != 0) {
                        return true;
                    }
                    results.push(this.found(word, ru));
                }
                if (langs.includes(Lang.ENGLISH) && !en.exclude.includes(word)) {
                    if (en.include.filter((w) => word === w || word.includes(w)).length != 0) {
                        return true;
                    }
	 ...
	 */
	filter(string: string = "", options: AutomodOptions = this.defaultOpts): AutomodReply {
    ...
    private clean(reply: AutomodReply, options: AutomodOptions): AutomodReply {
        ...
        for(let word of reply.output.split(/ /)) {
    ...
    private found(string: string, lang: any, index: number = 0): boolean {
        let replacers: string[] | undefined = lang.replacers[string.charAt(index)];
        if (replacers) {
            let results: boolean[] = []
            for(let i = 0; i < replacers.length; i++) {
                let newString = string.replace(string.charAt(index), replacers[i]);
                if (lang.include.filter((w: string) => newString === w || newString.includes(w)).length != 0) {
                    return true;
                }
